...
document.addEventListener('DOMContentLoaded', function () {
  chrome.storage.local.get(['githubToken', 'selectedRepo', 'nickname', 'savedText', 'savedTemplate', 'habit', 'submissionDate'], /*#__PURE__*/function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(result) {
      var textarea, habitSection, prevDate, _getDateInformation, _getDateInformation2, year, month, day;
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            // 깃헙 토큰이 있다면 로그인된 상태.
            if (result.githubToken) {
              ...
              if (result.selectedRepo) {
                ...
                habitSection = document.getElementById('extension-optional-habit-article');
                if (result.habit) {
                  // 오늘 회고 작성했는지 여부 볼 수 있음.
                  habitSection.style.display = 'flex';
                  prevDate = result.submissionDate;
                  _getDateInformation = (0,_utils_getDate__WEBPACK_IMPORTED_MODULE_3__.getDateInformation)(), _getDateInformation2 = _slicedToArray(_getDateInformation, 3), year = _getDateInformation2[0], month = _getDateInformation2[1], day = _getDateInformation2[2];
                  if (prevDate !== "".concat(year).concat(month).concat(day)) {
                    // 오늘 제출 안했다면?
                    habitSection.textContent = "".concat(year, "\uB144 ").concat(month, "\uC6D4 ").concat(day, "\uC77C \uD68C\uACE0\uB97C \uC791\uC131\uD558\uC9C0 \uC54A\uC558\uC5B4\uC694! \uD83D\uDE10");
                  } else {
                    // 오늘 제출했다면?
                    habitSection.textContent = "".concat(year, "\uB144 ").concat(month, "\uC6D4 ").concat(day, "\uC77C \uD68C\uACE0\uB97C \uC791\uC131\uD588\uC5B4\uC694! \uD83D\uDCAF");
                  }
                } else {
  ...
  document.getElementById('extension-logout-button').addEventListener('click', function () {
    chrome.storage.local.remove(['githubToken', 'selectedRepo', 'nickname', 'savedText', 'savedTemplate', 'habit', 'submissionDate'], function () {
      (0,_visibilities_setLogoutScreen__WEBPACK_IMPORTED_MODULE_6__.setLogoutScreen)();
    });
  });
...
function createFileAndCommit(token, repoName, fileName, content, nickname) {
  ...
  function createNewFile(token, repoName, fileName, content, nickname, folderPath) {
    fetch("https://api.github.com/repos/".concat(nickname, "/").concat(repoName, "/contents/").concat(folderPath, "/").concat(fileName), {
      ...
      if (response.status === 201) {
        var submissionDate = (0,_utils_getDate__WEBPACK_IMPORTED_MODULE_3__.getInitialFileName)();
        chrome.storage.local.set({
          'submissionDate': submissionDate
        });
        var habitSection = document.getElementById('extension-optional-habit-article');
        habitSection.textContent = "".concat(year, "\uB144 ").concat(month, "\uC6D4 ").concat(day, "\uC77C \uD68C\uACE0\uB97C \uC791\uC131\uD588\uC5B4\uC694! \uD83D\uDCAF");
  ...
  var _getDateInformation3 = (0,_utils_getDate__WEBPACK_IMPORTED_MODULE_3__.getDateInformation)(),
    _getDateInformation4 = _slicedToArray(_getDateInformation3, 3),
    year = _getDateInformation4[0],
    month = _getDateInformation4[1],
    day = _getDateInformation4[2];
